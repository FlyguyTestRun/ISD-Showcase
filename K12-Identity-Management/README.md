...
This folder contains PowerShell automation for managing student and staff identity lifecycle in K-12 educational environments. The **KISDIdentity** module implements Keller ISD naming conventions (`students.keller.edu`) and organizational unit (OU) structures aligned with educational workflows.
...
- **Hybrid Identity:** On-premises AD synced to Azure AD via Azure AD Connect
- **License Assignment:** Students = M365 A1 (free), Staff = M365 A3 (paid)
- **Group Membership:** Security groups synced to Microsoft 365 for Teams access
...
5. **Chromebook Distribution:** Students receive devices with pre-configured accounts
...
- **Annual Enrollment:** 1,200 students processed in ~12 minutes (vs. 300 hours manual)
...
**Planned Features:**
- Azure AD-native version (eliminate on-prem AD dependency for cloud-first districts)
- Intune integration (auto-assign Chromebooks/devices to student accounts)
- Self-service password reset portal for students (reduce help desk tickets)
- SIS API integration (eliminate CSV export step, real-time sync)
- Machine learning for duplicate detection (fuzzy matching on similar names)
...
**Purpose:** Demonstration of K-12 identity management expertise for Keller ISD Senior Systems Engineer position

**Note:** This module represents a working proof-of-concept using Keller ISD naming conventions. Production deployment would require customization for specific district requirements (OU structure, SIS integration, licensing, etc.).
