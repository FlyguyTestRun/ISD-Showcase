...
.DESCRIPTION
    Creates realistic CSV datasets for DHCP scope utilization and network device inventory
    for K-12 network monitoring at large-district scale.
    Data is calibrated to a 51-square-mile district with 40+ campuses and facilities:
    ~850 access points, ~220 access switches, ~18 distribution switches.
...
Write-Host ""
Write-Host "=== Generating Network Infrastructure Sample Data ===" -ForegroundColor Cyan

# 1. DHCP Scope Utilization
Write-Host ""
Write-Host "[1/2] Generating DHCP scope utilization data..." -ForegroundColor Yellow
...
for ($i = 0; $i -lt 30; $i++) {
    ...
    # Utilization higher on weekdays (students and staff on campus)
    ...
    foreach ($scope in $vlanScopes) {
        ...
        $utilization = [Math]::Max(0.05, [Math]::Min(0.95, $utilization))
...
# Scale: 40+ campuses, ~850 APs, ~220 access switches, ~18 distribution switches
Write-Host ""
Write-Host "[2/2] Generating network device inventory..." -ForegroundColor Yellow

$campuses = @(
    "Keller HS", "Timber Creek HS", "Central HS", "Foster HS",
    "Indian Springs MS", "Hillwood MS", "Trinity Springs MS", "Bear Creek MS", "Timberview MS",
    "Shady Grove Elem", "Parkwood Elem", "Willowbrook Elem", "Ridgeview Elem", "Willis Lane Elem",
    "Florence Elem", "Basswood Elem", "Parkview Elem", "Liberty Elem", "North Star Elem",
    "Administrative Center", "Transportation Center", "Stadium Complex"
)

$deviceTypes = @(
    @{Type="Core Switch";          Count=4;   Model="Cisco Catalyst 9500"}
    @{Type="Distribution Switch";  Count=18;  Model="Cisco Catalyst 9300"}
    @{Type="Access Switch";        Count=220; Model="Cisco Catalyst 9200"}
    @{Type="Wireless Controller";  Count=6;   Model="Cisco 9800"}
    @{Type="Access Point";         Count=850; Model="Cisco Catalyst 9120AXI"}
    @{Type="Firewall";             Count=8;   Model="Palo Alto PA-5220"}
    @{Type="Router";               Count=12;  Model="Cisco ISR 4451"}
)
...
Write-Host ""
Write-Host "Device Health Summary:" -ForegroundColor White
