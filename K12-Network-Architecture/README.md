...
### 1. `KISD-Network-Segmentation.md`
...
1. **Change VLAN Default Gateway Passwords** - Core switch management interface secured
2. **Implement 802.1X for All Networks** - Except guest (use captive portal)
3. **Separate IoT by Device Type** - VLAN 41 cameras, VLAN 42 HVAC (further isolation)
4. **Regular Firmware Updates** - Switches, firewalls, access points (schedule during breaks)
5. **Audit Firewall Rules Annually** - Remove obsolete rules, verify least-privilege access
6. **Test DR Procedures** - Restore DHCP/DNS from backup, verify network services recover
7. **Document Changes** - Maintain network diagram and change log in version control
...
**Purpose:** Demonstration of K-12 network architecture expertise for Keller ISD Senior Systems Engineer position
